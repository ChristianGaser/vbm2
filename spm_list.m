...
h  = text(0.02,y-9*dy/8,	'\itp ');			Hc = [Hc,h];
								Hp = [Hp,h];
if isfield(varargin{2},'VRpv')
	ht = text(0.27,y, 'non-stationarity adjusted cluster-level','FontSize',FS(9));
	set(ht,'HorizontalAlignment','Center');
else
	text(0.22,y, 'cluster_level','FontSize',FS(9));
end
...
if isfield(varargin{2},'VRpv')
	try
		[N Z XYZ A V2R] = spm_max_nS(zscores,varargin{2}.XYZ,varargin{2}.VRpv);
		V2R = V2R./N;
...
[tmpRs itmpRs jtmpRs]   = unique(N); %-shorten the list of cluster sizes
                                      % in resels
