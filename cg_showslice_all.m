...
% @(#)cg_showslice_all.m    Christian Gaser 1.02 2006/12/29
...
while (sl < 1) | (sl > V(1).dim(3))
	s = spm_input(['Slice (in mm) [' num2str(range(1)) '...' num2str(range(2)) ']'],1,'e',0);
	sl = s/vx(3)+Orig(3);
end
   ...
   for j = 1:sizey
	...
	if k < n
	  text(round(sizex*V(1).dim(1)-((i-1)*V(1).dim(1)+(i)*V(1).dim(1))/2),(j-1)*V(1).dim(2)+fs+2,names.m{k+1},...
	  	'FontSize',fs,'Color','y','HorizontalAlignment','center');
	end
  end
end

axis off;
title(sprintf('Common filename: %s*%s',names.s, names.e),'FontSize',FS(10),'FontWeight','Bold');
