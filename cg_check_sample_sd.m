...
% @(#)cg_check_sample_sd.m	v1.02 Christian Gaser 2006/12/29

spm5 = 0;
if strcmp(spm('ver'),'SPM5'), spm5 = 1; end

if spm5
	P = spm_select(Inf,'w.*\.img$','Select images');
else
	P = spm_get(Inf,'*w*.img','Select normalized files');
end
...
writeout = spm_input('Write SD file?','+1','yes|no',[1 0],2);

if writeout
	% remove trailing directory to keep only filenames
	[tmp, filesummary] = spm_str_manip(spm_get('FileSummary',spm_str_manip(P,'t')),'C');
	if ~isempty(filesummary)
        Q = ['sd_' filesummary.s filesummary.e];
	else
	   Q = ['sd_' spm_str_manip(tmp,'v')];
    end
...
set(gca,'XTick',[],'XLim',[-.25 2.25],'YLim',[0.9*min(squared_distance) 1.1*max(squared_distance)]);
...
%-----------------------------------------------------------------------
if writeout
	Vo = spm_write_vol(Vo,std_data);
	disp(['Writing ' Vo.fname]);
end
