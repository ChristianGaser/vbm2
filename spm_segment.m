...
% modified to add HMRF options by Christian Gaser
%
...
if isfield(flags.write,'mrf')
    if flags.write.mrf 
	   vx = sqrt(sum(VF(1).mat(1:3,1:3).^2));
	   [g,w,c] = cg_gwc_HMRF(g,w,c,flags.write.mrf,vx);
    end	   
end;
...
for j=1:3,
    ...
    if isfield(flags.write,'mrf')
        if flags.write.mrf
    	   tmp   = fullfile(pth,[nm '_seg' num2str(j) '_HMRF' xt]);
        end
	end
	VO(j) = struct(...
		'fname',tmp,...
